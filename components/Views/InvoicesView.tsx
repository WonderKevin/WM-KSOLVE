...
function parseMetadataFromText(text: string) {
  ...
  if (invoice === "Unknown") {
    const m =
      ...
      norm.match(/\b([A-Z]{0,10}\d[A-Z0-9.\-\/]{1,})\b/);
...
async function safeExtractPdfText(file: File) {
  ...
  if (detectedType === "excel") {
    throw new Error(`${file.name}: this file is an Excel spreadsheet, not a PDF.`);
  }

  try {
    return await extractTextWithPdfJs(file);
  } catch (err: any) {
    throw new Error(`${file.name}: failed to read PDF. ${err?.message || "Invalid PDF file."}`);
  }
}
...
async function fetchInvoiceType(invoiceNumber: string): Promise<string> {
  ...
  const { data: invData } = await supabase.from("invoices").select("type, invoice_number").limit(5000);
  const matchInInvoices = (invData || []).find((r) => normalizeInvoiceNumber(r.invoice_number || "") === norm);
  if (matchInInvoices?.type) return matchInInvoices.type;

  const { data: upData } = await supabase.from("uploads").select("category, invoice").limit(5000);
  const matchInUploads = (upData || []).find((r) => normalizeInvoiceNumber(r.invoice || "") === norm);
  return matchInUploads?.category || "";
}
...
async function fetchInvoiceAmount(invoice: string): Promise<number> {
  ...
  const { data } = await supabase.from("invoices").select("invoice_number, invoice_amt").limit(5000);
...
async function fetchInvoiceCheckDate(invoice: string): Promise<string> {
  ...
  const { data } = await supabase.from("invoices").select("invoice_number, check_date").limit(5000);
  const matched = (data || []).find((r) => normalizeInvoiceNumber(r.invoice_number || "") === ni);
  return isoDateToMmDdYyyy(matched?.check_date || "");
}

function extractWMInvoiceGrandTotal(text: string): number | null {
  const lines = text
    .replace(/\u00a0/g, " ")
    .replace(/\r/g, "\n")
    .replace(/[ \t]+/g, " ")
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/^total$/i.test(lines[i]) || /^invoice total$/i.test(lines[i])) {
  ...
  const allAmounts = lines.flatMap((line) => [...line.matchAll(/\$?\s*([\d,]+\.\d{2})/g)].map((m) => m[1]));
  if (!allAmounts.length) return null;

  return parseFloat(allAmounts[allAmounts.length - 1].replace(/,/g, ""));
}
...
function parseFreshThymeSasPdfRows(text: string): DatasetRow[] {
  ...
  for (const l of lines) {
    if (/ep\s*fee/i.test(l)) {
      ...
      if (m) epFee = parseFloat(m[1].replace(/,/g, ""));
    }
  }
  ...
  for (const l of lines) {
    ...
    raw.push({ upc: um[1], qty: nums.length ? parseInt(nums[nums.length - 1], 10) : 1, amt: parseFloat(am[1].replace(/,/g, "")) });
  }
...
function parseWMInvoicePdfRows(text: string): DatasetRow[] {
  ...
  let maAmount = 0;
  for (let i = 0; i < lines.length; i++) {
    if (/^ma\s*2%$/i.test(lines[i]) || /ma\s*2%/i.test(lines[i])) {
  ...
  const isSku = (line: string) => /^[A-Z]{2,6}-[A-Z0-9]{2,12}-\d{2,4}$/i.test(line);

  for (let i = 0; i < lines.length; i++) {
    const sku = lines[i];
    if (!isSku(sku) || seen.has(sku)) continue;
    ...
    for (let j = i + 1; j < Math.min(i + 12, lines.length); j++) {
      ...
      if (isSku(line) && j > i + 1) break;
      if (/^ma\s*2%$/i.test(line) || /^total$/i.test(line)) break;

      if (qty === null && /^\d+$/.test(line)) {
      ...
      if (amountMatch) {
        lineTotal = parseFloat(amountMatch[1].replace(/,/g, ""));
      }
    }

    if (qty === null || lineTotal === null || lineTotal <= 0) continue;

    raw.push({ upc: sku, qty, amt: lineTotal });
    seen.add(sku);
  }
  ...
  const productSubtotal = raw.reduce((sum, r) => sum + r.amt, 0);
  const grandTotal = extractWMInvoiceGrandTotal(text);

  for (const r of raw) {
    const maShare = totalQty > 0 ? (r.qty / totalQty) * maAmount : 0;
    const netLine = Math.round((r.amt + maShare) * 100) / 100;

    rows.push({
      upc: r.upc,
      item: "",
      cust_name: customer,
      amt: Math.abs(netLine),
    });
  }

  const netTotal = rows.reduce((sum, r) => sum + r.amt, 0);
  if (grandTotal !== null && Math.abs(netTotal - grandTotal) > 1.0 && Math.abs(productSubtotal - grandTotal) > 1.0) {
    throw new Error(`Wonder Monday parse mismatch. Parsed total ${netTotal.toFixed(2)} does not match invoice total ${grandTotal.toFixed(2)}.`);
  }
...
function parseExcelProductDetailsRows(buffer: ArrayBuffer): DatasetRow[] {
  ...
  const ak = keys.find((k) => /scanned\s*sales|bill\s*amount/i.test(k)) ?? "";
  const uk = keys.find((k) => /^upc$/i.test(k)) ?? "";
  const qk = keys.find((k) => /qty\s*ship|^qty$/i.test(k)) ?? "";

  let epFee = 0;
  for (const sheetName of wb.SheetNames) {
    const allRows = XLSX.utils.sheet_to_json<any[]>(wb.Sheets[sheetName], { header: 1, raw: false, defval: "" });
    for (let ri = 0; ri < allRows.length && !epFee; ri++) {
...
async function parseDetailRows(file: File, fullText?: string): Promise<DatasetRow[]> {
  if (await isExcelFile(file)) return parseExcelProductDetailsRows(await file.arrayBuffer());
  ...
  if (/wonder\s*monday/i.test(lt) && /ship\s+to/i.test(lt) && /kehe\s+distributors/i.test(lt) && /invoice\s+no/i.test(lt)) return parseWMInvoicePdfRows(text);
  if (/distributor\s+charge/i.test(lt) && /sold\s+to/i.test(lt)) return parseDollarPromotionPdfRows(text);
  if (/new\s+item\s+set\s*up/i.test(lt) && /invoice\s+total/i.test(lt) && /dc\s*#/i.test(lt)) return parseNewItemSetupPdfRows(text);
...
async function syncInvoiceFromUpload(invoice: string, type: string) {
  ...
  const ni = normalizeInvoiceNumber(invoice);
  const { data } = await supabase.from("invoices").select("id, invoice_number").limit(5000);
  const matched = (data || []).find((r) => normalizeInvoiceNumber(r.invoice_number || "") === ni);
  if (!matched) return;
...
async function syncInvoiceAmountFromDocument(invoice: string, amount: number | null) {
  if (!invoice || invoice === "Unknown" || amount === null) return;
  ...
  await supabase.from("invoices").update({ invoice_amt: amount }).eq("id", matched.id);
}
...
export default function InvoicesView({
  ...
  const uploadNewDocument = async (
    ...
    const { error: se } = await supabase.storage.from(DOCUMENT_BUCKET).upload(fp, file, {
      cacheControl: "3600",
      upsert: false,
      contentType: ct,
    });
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          let parsedInvoiceTotal: number | null = null;
          if (detectedType === "pdf" && meta.category === "WM Invoice") {
            const { fullText } = await safeExtractPdfText(file);
            parsedInvoiceTotal = extractWMInvoiceGrandTotal(fullText);
          }
          ...
          if (parsedInvoiceTotal !== null && finalMeta.category === "WM Invoice") {
            await syncInvoiceAmountFromDocument(finalMeta.invoice, parsedInvoiceTotal);
          }
  ...
  const documentFilterLabel = documentFilter === "Documents" ? "Documents" : documentFilter;
      ...
      {reprocessModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
            ...
            <p className="mb-4 text-sm text-slate-500">Choose a date range. Leave blank to reprocess all.</p>
            ...
            <div className="flex gap-3">
              ...
              <button type="button" onClick={doReprocess} className="flex-1 rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700">{reprocessFrom || reprocessTo ? "Reprocess Range" : "Reprocess All"}</button>
            </div>
          </div>
        </div>
      )}

      {(deductionModal.open || upcModal.open) && (
        <div className="fixed inset-0 z-[150] bg-black/30" />
      )}
          ...
          <CardContent className="pt-6">
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-7">
              ...
              <div className="relative" ref={documentDropdownRef}>
                <button type="button" onClick={() => setDocumentDropdownOpen((p) => !p)} className="flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                ...
                {documentDropdownOpen && (
                  <div className="absolute z-30 mt-2 w-full rounded-xl border border-slate-200 bg-white p-1 shadow-lg">
                    {["Documents", "With Document", "Without Document"].map((label) => (
                      <button key={label} type="button" onClick={() => { setDocumentFilter(label); setDocumentDropdownOpen(false); }} className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm hover:bg-slate-50 ${documentFilter === label ? "bg-slate-50" : ""}`}>
                        <span>{label}</span>
                        {label === "Without Document" && withoutDocumentCount > 0 && (
              ...
              <div className="flex gap-2">
                <Button type="button" variant={selectMode ? "default" : "outline"} onClick={() => {
                  const next = !selectMode;
                  setSelectMode(next);
                  if (!next) { setSelectedIds([]); setDeleteMonth("Delete Month"); }
                }} className="flex-1">
                  Select
                </Button>
                  ...
                  {filteredRows.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className="border-t">
                        ...
                        <td className="px-4 py-3">
                          {hasDoc ? (
                            <button type="button" onClick={() => openDocumentByInvoice(row.invoice_number)} className={docType === "excel" ? "text-green-600 hover:text-green-700" : "text-red-500 hover:text-red-700"}>
                              {docType === "excel" ? <FileSpreadsheet className="h-5 w-5" /> : <FileText className="h-5 w-5" />}
                            </button>
                          ) : (
                            <span className="text-slate-300"><XCircle className="h-5 w-5" /></span>
                          )}
                        </td>
                      </tr>
                    );
