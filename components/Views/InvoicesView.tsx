...
async function extractDocumentMetadata(
  ...
  if (ln.endsWith(".xlsx") || ln.endsWith(".xls")) {
...
async function replaceDatasetRowsForInvoice(invoice: string, pdfDate: string, file: File, categoryFallback = "") {
  ...
  if (!ni) return 0;
  const ln = file.name.toLowerCase();
  const isExcel = ln.endsWith(".xlsx") || ln.endsWith(".xls");
  let detailRows: DatasetRow[] = [];
  if (isExcel) {
    detailRows = parseExcelProductDetailsRows(await file.arrayBuffer());
  } else {
  ...
  const finalType = it || categoryFallback;
  const inserts = detailRows.map((r) => ({
    month: formatMonthLabelFromDate(pdfDate),
    invoice: ni,
    invoice_date: parseIsoDateFromMmDdYyyy(pdfDate),
    type: finalType,
    upc: r.upc,
    item: pl.get(r.upc) || r.item,
...
async function reprocessAllUploads(onProgress: (msg: string) => void): Promise<{ processed: number; failed: number; totalRows: number }> {
  ...
  for (const u of all ?? []) {
    ...
    try {
      ...
      const f = new File([fd], u.file_name, {
        type:
          u.file_type === "excel"
...
function getDocumentIcon(upload: UploadRecord | undefined) {
  if (!upload) return <XCircle className="h-5 w-5 text-red-600" />;
...
export default function InvoicesView({
  ...
  const handleDocumentChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await loadData();
      if (ok > 0 || rep > 0 || skip > 0) {
        showToast(`${ok} uploaded, ${rep} replaced, ${skip} skipped, ${savedRows} dataset rows saved.`, "success");
      }
    } catch (e: any) {
                        ...
                        <td className="px-4 py-3">
                          {hasDoc ? (
                            <button
                              ...
                              title={ur?.file_type === "excel" ? "Open Excel" : "Open PDF"}
                            >
                              {getDocumentIcon(ur)}
                            </button>
                          ) : (
                            getDocumentIcon(undefined)
                          )}
                        </td>
                      </tr>
                    );
