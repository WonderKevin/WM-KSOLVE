...
function parseWMInvoicePdfRows(text: string): DatasetRow[] {
  ...
  const tq = raw.reduce((s, r) => s + r.qty, 0);
  for (const r of raw) {
    rows.push({ upc: r.upc, item: "", cust_name: customer, amt: Math.round((r.amt + (tq > 0 ? r.qty / tq * maAmount : 0)) * 100) / 100 });
  }
  return rows;
}
...
function parseExcelProductDetailsRows(buffer: ArrayBuffer): DatasetRow[] {
  ...
  for (const row of json) {
    ...
    const qty = parseAmount(row[qk]) ?? 0;
  ...
  console.log("[ExcelProductDetails] epFee:", epFee, "| totalQty:", totalQty, "| rows:", rows.length);
  return rows;
}
