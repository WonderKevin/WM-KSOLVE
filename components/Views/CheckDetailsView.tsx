        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {checkGroups.map((group) => {
              ...
              const typeEntries = Array.from(typeMap.entries()).map(([typeName, typeRows]) => ({
                typeName,
                totalAmount: typeRows.reduce((sum, item) => sum + (Number(item.invoice_amt) || 0), 0),
                rows: typeRows,
              }));

              return (
                <div key={checkKey} className="overflow-hidden rounded-3xl border border-slate-200 bg-white">
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="mt-1 font-semibold text-slate-900">{group.month}</div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold uppercase tracking-wide text-slate-400">Check Date</div>
                      <div className="mt-1 font-semibold text-slate-900">{group.check_date}</div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold uppercase tracking-wide text-slate-400">Check #</div>
                      <div className="mt-1 font-semibold text-slate-900">{group.check_number}</div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold uppercase tracking-wide text-slate-400">Check Amt</div>
                      <div className="mt-1 font-semibold text-slate-900">{formatCurrency(group.check_amt)}</div>
                    </div>
                  </button>
                    ...
                    <div className="border-t border-slate-200 bg-slate-50 p-4">
                      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                        ...
                        {typeEntries.map((entry) => {
                          ...
                          return (
                            <div key={typeKey} className="border-t border-slate-200 first:border-t-0">
                              <button
                                type="button"
                                onClick={() => toggleType(typeKey)}
                                className={`grid w-full grid-cols-[48px_1fr_220px] items-center px-4 py-4 text-left transition ${
                                  isTypeOpen ? "bg-slate-100" : "bg-white hover:bg-slate-50"
                                }`}
                              >
                                <div>
                                  {isTypeOpen ? (
                                ...
                                <div className="font-medium text-slate-900">{entry.typeName}</div>
